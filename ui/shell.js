/* ── Bus Adventures — ui/shell.js ────────────────────────────────────────────
   Injects the full game UI into <body> and wires all shared interaction.
   Called once per game after configure() and startGame().

   Usage:
     import { initShell } from "../../ui/shell.js";
     initShell({ handleCommand, game });

   game object must contain:
     game.title        — shown in about overlay
     game.version      — e.g. "v1.0"
     game.about        — array of paragraph strings
     game.contact      — email address
   ─────────────────────────────────────────────────────────────────────────── */
...
<div id="aboutOverlay" class="about-overlay hidden">
  <div class="about-panel">
    ...
    <div class="about-content">
      ...
      <div id="shellAboutBody"></div>
      <div class="about-divider">— CONTACT —</div>
      <a class="about-contact" id="shellAboutContact" href=""></a>
...
`);
  ...
  // ── Populate game-specific text ───────────────────────────────────────────
  const g = game;
  const intro = g.intro || {};
  const win   = g.win   || {};

  if (intro.title)      document.getElementById('shellGameTitle').textContent    = intro.title;
  if (intro.subtitle)   document.getElementById('shellGameSubtitle').textContent = intro.subtitle;
  if (intro.startLabel) document.getElementById('startBtn').textContent          = intro.startLabel;
  if (win.title)        document.getElementById('shellWinTitle').textContent     = win.title;
  if (win.text)         document.getElementById('shellWinText').textContent      = win.text;
  if (win.playAgainLabel) document.getElementById('playAgainBtn').textContent    = win.playAgainLabel;

  if (intro.text) {
    const el = document.getElementById('shellIntroParagraphs');
    el.innerHTML = intro.text.split('\n\n').map(p => `<p>${esc(p)}</p>`).join('');
  }
  if (g.version) {
    document.getElementById('shellAboutVersion').textContent = `${g.version} — ${intro.title || g.title || ''}`;
  }
  if (g.about) {
    const el = document.getElementById('shellAboutBody');
    el.innerHTML = g.about.map(p => `<p class="about-body">${esc(p)}</p>`).join('');
  }
  if (g.contact) {
    const a = document.getElementById('shellAboutContact');
    a.textContent = g.contact;
    a.href = `mailto:${g.contact}`;
  }
